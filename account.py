...
class Account:
  ...
  def deposit(self, amount = None, external_transaction = None):
    if amount is None:
  ...
  def withdraw(self, amount = None, external_transaction = None):
    if amount is None:
      while not (amount_to_withdraw := input('Amount to withdraw: ')).isdigit(): # Ser till så att mängden är en siffra
